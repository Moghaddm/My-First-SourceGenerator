...
public class ServiceGenerator : ISourceGenerator
{
    ...
    public void Execute(GeneratorExecutionContext context)
    {
        if (
            ...
            is not AttributeSyntaxReceiver<GenerateServiceAttribute> syntaxReceiver
        )
        {
            return;
        }
                ...
                .First(
                    x =>
                        x.Name
                            ...
                            .Equals(typeof(GenerateServiceAttribute).Name)
                );

            // Getting constructor parameter of the attribute. It might be not presented.
            var templateParameter = attribute.ArgumentList?.Arguments
                .FirstOrDefault()
                ?.GetLastToken()
